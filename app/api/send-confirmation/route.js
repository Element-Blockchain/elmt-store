  ...
  return cart.map(item => `
    <tr>
      <td style="padding:10px 0;border-bottom:1px solid #2a2a2a;vertical-align:middle;">
        ${item.img
          ? `<img src="${item.img}" alt="${item.name}" width="48" height="48" style="border-radius:8px;object-fit:contain;background:#1a1a1a;display:block;"/>`
          : `<div style="width:48px;height:48px;background:#1a1a1a;border-radius:8px;font-size:24px;text-align:center;line-height:48px;">${item.icon || '📦'}</div>`
        }
      </td>
...
function txSection(txHash, chainId) {
  if (!txHash) return ''
  const explorerUrl = chainId === 1
    ? 'https://etherscan.io/tx/' + txHash
    : 'https://basescan.org/tx/' + txHash
  const explorerLabel = chainId === 1 ? 'View on Etherscan' : 'View on BaseScan'
  return `
    <div style="background:#1a1a1a;border:1px solid #2a2a2a;border-radius:8px;padding:16px;margin:20px 0;">
      ...
      <a href="${explorerUrl}" style="color:#888;font-size:11px;text-decoration:none;">${explorerLabel}</a>
    </div>
  `
}
...
function customerTemplate({ customerName, orderId, cart, paymentMethod, total, txHash, chainId, shippingAddress }) {
  return `<!DOCTYPE html>
<html>
...
function internalTemplate({ customerEmail, customerName, orderId, cart, paymentMethod, total, txHash, chainId, shippingAddress }) {
  return `<!DOCTYPE html>
<html>
<body style="background:#0d0d0d;margin:0;padding:40px 20px;font-family:Inter,sans-serif;">
  <div style="max-width:560px;margin:0 auto;">
  ...
  try {
    ...
    for (const email of INTERNAL_EMAILS) {
      await sendEmail({
        ...
        html: internalTemplate({ customerEmail, customerName, orderId, cart, paymentMethod, total, txHash, chainId, shippingAddress }),
      })
    }
